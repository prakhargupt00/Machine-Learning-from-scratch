# -*- coding: utf-8 -*-
"""
Created on Thu Oct 31 16:20:56 2019
...
#Lamda
lamdas = np.linspace(0,100,num=20)
...
for lamda in lamdas:
    ...
    #calculating the error function using gradient descent with l1 norm regularisation 
    for i in range(k):   #num_of_iterations
        ...
        #updating wieghts
        w[0]=w[0]-alpha*(c0) - alpha*(math.copysign(lamda,w[0]))
        w[1]=w[1]-alpha*(c1) - alpha*(math.copysign(lamda,w[1]))
        w[2]=w[2]-alpha*(c2) - alpha*(math.copysign(lamda,w[2]))
    ...
    r2_values.append(1-res_sum/tot_sum)
